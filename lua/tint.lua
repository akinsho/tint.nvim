...
local function setup_autocmds()
  ...
  vim.api.nvim_create_autocmd({ "WinEnter" }, {
    group = augroup,
    pattern = { "*" },
    callback = __.on_winenter,
  })

  vim.api.nvim_create_autocmd({ "WinLeave" }, {
    group = augroup,
    pattern = { "*" },
    callback = __.on_winleave,
  })
...
--- Triggered by `:h WinEnter`, restore the default highlight namespace
__.on_winenter = function()
  vim.api.nvim_win_set_hl_ns(vim.api.nvim_get_current_win(), __.default_ns)
end

--- Triggered by `:h WinLeave`, set the tinted namespace
__.on_winleave = function()
